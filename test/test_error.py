...
def default_importerror(obj):
    import doesnotexist  # type: ignore
...
class TestJsonEncodeError:
    def test_dumps_arg(self):
        ...
        assert (
            str(exc_info.value)
            == "dumps() missing 1 required positional argument: '__obj'"
        )
    ...
    def test_dumps_chain_none(self):
        ...
        # assert exc_info.value.__cause__ is None
        assert str(exc_info.value.__cause__) == "Type is not JSON serializable: Custom"
